...
電気的接点が物理的に閉／開する時に、この跳ね返りが原因で、機械的な弾性振動が発生します。これが乱雑な電気的パルスを発生させる。（パルス幅の前後にぐにゃぐにゃしたものがあったらチャタリング）
...
- ボーレートは「1秒あたりのビット数」
マイコン内部のクロックを分周して生成するため、設定値との間に誤差が生じます。

## TCP/IP モデルとネットワークの基礎

ネットワーク通信は「役割ごとに層（レイヤー）に分けて」設計されています。

![alt text](image-1.png)

組み込み開発では第4層を使う

- IP アドレスは 32 ビットの数値を 8 ビットずつ 4 つに区切って表します。
  2の32乗は10進数で表すと4294967296
  これを256ずつに区切る

## IPv6

アドレス空間が枯渇してしまったために新しく増築された。
アドレス数が10の38乗もある。

FQDN とは、ホスト名からルートまでのすべての階層を含む完全なドメイン名。

     www        .    example    .    com    .
  ホスト名          ドメイン名       TLD    ルート（省略可）

## 卒業制作

## 電子楽器（リコーダーなどの木管楽器）

なんでもいいので管にボタンが付いている状態。
ボタンを押した上で息を吹き込むとボタンで選択した音が反応する（鍵盤ハーモニカ方式）
→風力を入力する機械はあるのだろうか。。。？
管の中にプロペラのようなものを入れれば可能。(プロペラから入力を行う)
...
[中国SiCの進化「日本は追い付けないレベル」競わず活用を](https://eetimes.itmedia.co.jp/ee/articles/2605/27/news056.html)

## 感想/質問/その他(振り返り)

卒業制作は如何せんいかようにもやりようがありそうなので思いついたやり方からやっていくとする。
そこら辺のビニールパイプにボタンとプロペラをつけただけのものを作品と呼ぶのはかなり粗暴な気がする点は否めないがとりあえずのプロトタイプということで行ける所までやってみよう。
